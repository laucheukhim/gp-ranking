...
const practices = await new Promise((resolve, reject) => {
  ...
  fs.createReadStream(
    decodeURIComponent(
      new URL('../../data/practices.csv', import.meta.url).pathname,
    ),
  )
    .pipe(csv())
    .on('data', data =>
...
for (let i = 0, length = practices.length; i < length; i++) {
  ...
  console.log(`${i + 1}/${length} ${practice.practiceName}`);
  ...
  root.getElementsByTagName('script').forEach(element => {
    ...
    if (script.includes('openstreetmap')) {
      ...
      const latNode = getVariableByName(tree, 'lat');
      if (latNode) {
        practice.latitude =
          latNode.type === 'UnaryExpression'
            ? -latNode.argument.value
            : latNode.value;
      }
      const longNode = getVariableByName(tree, 'long');
      if (longNode) {
        practice.longitude =
          longNode.type === 'UnaryExpression'
            ? -longNode.argument.value
            : longNode.value;
      }
